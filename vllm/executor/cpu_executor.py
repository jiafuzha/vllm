...
class CPUExecutor(ExecutorBase):
    ...
    def __init__(
        ...
        # ====NS changes to check quantization and neural-speed availability
        assert model_config.quantization == "neuralspeed", "CPUExecutor only supports quantization neuralspeed."
        # check if neuralspeed is available
        spec = importlib.util.find_spec("neural_speed")
        assert spec is not None, "neuralspeed is not available. Please install neuralspeed with 'pip install neural-speed'."
    ...
    def _init_worker(self):
        ...
        assert self.parallel_config.world_size == 1, (
            "CPUExecutor only supports single CPU.")

        # ====NS deletes since no distribution in NS
        ...
        self.driver_worker = Worker(
            ...
            local_rank=0,
            rank=0,
            # ====NS keeps and sets to None for later deep integration
    ...
    def _init_cache(self) -> None:
        ...
        TODO: expose neural speed cache to python
        """
        pass
