"""A CPU worker class."""
# ====NS deletes some useless imports
# import gc
# import os
...
from vllm.worker.cpu_model_runner import ModelRunner
from vllm.lora.request import LoRARequest

class Worker:
    """A worker class that executes (a partition of) the model on a CPU.

    Each worker is associated with a single CPU. The worker is responsible for
    ...
    def init_model(self) -> None:
        # ====NS deletes since we are using neural-speed
        ...
        # TODO: distribute in multiple sockets
    ...
    def profile_num_available_memory(self) -> Tuple[int, int]:
        ...
        free_cpu_memory, total_cpu_memory = ns.mem_get_info()
        # NOTE(woosuk): Here we assume that the other processes using the same
        # GPU did not change their memory usage during the profiling.
        # peak_memory = self.init_cpu_memory - free_cpu_memory
        ...
        ns.collect()
        return total_cpu_memory, free_cpu_memory

    def init_cache_engine(self, cache_config: CacheConfig) -> None:
        # ====NS deletes since KV cache managed internally in NS
        # self.cache_config = cache_config
        # self.cache_engine = CacheEngine(self.cache_config, self.model_config,
        #                                 self.parallel_config)
        # self.cache_events = self.cache_engine.events
        # self.cpu_cache = self.cache_engine.cpu_cache
        # self.model_runner.set_cpu_cache(self.cpu_cache)
        pass

    def warm_up_model(self) -> None:
        # ====NS deletes 
        # ====NS TODO: warm up model
...
# ====NS deletes since they are useless
    # def cache_swap(
    #     self,
    ...
    def execute_model(
        ...
        # ====NS changes to execute model in NS
        # if self.is_driver_worker:
