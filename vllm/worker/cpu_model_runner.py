...
from vllm.sequence import FinalSequenceOutputs, SequenceGroupOutput
from vllm.ns_structures import Generation


logger = init_logger(__name__)

KVCache = Tuple[any, any]
_PAD_SLOT_ID = -1
LORA_WARMUP_RANK = 8
...
class ModelRunner:
    ...
    def load_model(self) -> None:
        ...
        # self.model.init_from_bin is called on the first generate call

        # self.model_memory_usage = ns.consumed_memory
        ...
        gs_ptr = self.model.get_generations_address()
        self.ns_generations = ctypes.cast(gs_ptr, ctypes.POINTER(Generation))        
    ...
    # ) -> Tuple[any, any, InputMetadata, List[int], List[int],
    ...
    #     ]
    #     context_lens_tensor = any(context_lens,
    #                                        dtype=torch.int,
    #                                        device=self.device)
    ...
    #                                     dtype=torch.long,
    #                                     device=self.device)
    #     prompt_lens_tensor = any(prompt_lens,
    #                                       dtype=torch.long,
    #                                       device=self.device)
    ...
    #         block_tables = any(input_block_tables, device=self.device)
    #     else:
    ...
    # ) -> Tuple[any, any, InputMetadata, SamplingMetadata,
    #            Set[int], LoRAMapping]:
    ...
    def execute_model(
        ...
        # queries = self.prepare_input_tensors(seq_group_metadata_list)
        ...
        for seq_group_metadata in seq_group_metadata_list:
            if seq_group_metadata.is_prompt: # issue new generation request only, existing generations are managed in native for now.
                query_id = list(seq_group_metadata.seq_data.keys())[0] # first seq id
                ...
                g.max_new_tokens = seq_group_metadata.sampling_params.max_tokens
                ctypes.memmove(g.prompt_ids, prompt_tokens_c_arr, n_prompt_tokens*ctypes.sizeof(ctypes.c_int32))
            
            
        # ====NS execute model
        # out is a list of completed generation ids
        out = self.model()
        if out:
            # ====NS mimic sampler._build_sampler_output
            return self._build_sampler_output(out, seq_group_metadata_list)
    ...
    def _build_sampler_output(self, ns_output: List[int], seq_group_metadata_list: List[SequenceGroupMetadata]) -> SamplerOutput:
        # copy generated tokens, update vllm sequence, update generation status to consumed
        # only completed generations returned
        seq_2_gen = {}
        sampler_output = []
        sgm_size = len(seq_group_metadata_list)
        for gen_id in ns_output:
            gen = self.ns_generations[gen_id]
            gen.status = 4 # consumed, make sure not output lost. It's checked before ns generation structure is reused.
            token_ids = [gen.generated_ids[i] for i in range(gen.n_generated_tokens)]
            seq_2_gen[gen.query_id] = SequenceGroupOutput([FinalSequenceOutputs(gen.query_id, token_ids, None)], None)

        for sgm in seq_group_metadata_list: # only one seq in seq group
            seq_id = list(sgm.seq_data.keys())[0]
            if seq_id in seq_2_gen:
                sampler_output.append(seq_2_gen[seq_id])
            else: # pad output to align with seq_group_metadata_list
                sampler_output.append(SequenceGroupOutput([FinalSequenceOutputs(gen.query_id, [], None)], None))

        if len(sampler_output) != sgm_size:
            raise ValueError("ns output size mismatch, expected %d, got %d" % (sgm_size, len(sampler_output)))
